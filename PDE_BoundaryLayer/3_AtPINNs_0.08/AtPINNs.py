...
class PhysicsInformedNN():
    def __init__(self, Collo, lp, up, lf, ur, uv_layers, lb, ub, eps):
        ...
        self.mu_ = torch.tensor([0.8], requires_grad=True).to(device)
    ...
    def net_f(self, t, x):
        ...
        e1 = self.mu_*u_t - self.mu_*self.mu_*u_xx + u_x - 2*t
    ...
    def loss_func_lgbs(self):
        ...
        self.loss_mu.append(lossmu.item())
        ...
        if self.iter % 10 == 0:
            print('Iter %d, mu: %.5e, Loss: %.5e, loss_f: %.5e, loss_BC: %.5e' %
                  (self.iter, self.mu_.item(), loss.item(), lossf.item(), lossBC.item()))
    ...
    def LossSave(self, filename):
        data = np.array([self.iter_, self.mu_list, self.loss_mu, self.loss_save, self.loss_f, self.loss_BC])
        data = np.transpose(data)
        df = pd.DataFrame(data, columns=['iter', 'mu', 'Loss_mu', 'Loss', 'Loss_f', 'Loss_BC'])
    ...
    def predict_by_trained_1(self, data, filename):
        ...
        result[:, 2] = y_pred[:, 0]
        df = pd.DataFrame(result, columns=['t', 'x', 'u'])
        df.to_csv('./results/curve_results_'+str(self.iter)+'.csv', index=False, header=None)
