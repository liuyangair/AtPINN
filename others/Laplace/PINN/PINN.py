...
class PhysicsInformedNN():
    def __init__(self, Collo, BC, u_BC, uv_layers, lb, ub, eps):
        ...
        self.u_BC = torch.tensor(u_BC[:, 0:1], requires_grad=True).float().to(device)
    ...
    def loss_func_lgbs(self):
        ...
        self.loss_save_list.append(loss.item())
    ...
    def LossSave(self, filename):
        data = np.array([self.iter_list, self.loss_save_list, self.loss_f_list, self.loss_BC_list])
...
if __name__ == "__main__":
    ...
    model = PhysicsInformedNN(XY_c, BC, u_BC, uv_layers, lb, ub, eps)
    model.train_LBFGS()
