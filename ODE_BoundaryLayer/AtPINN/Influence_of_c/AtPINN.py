...
class PhysicsInformedNN():
    def __init__(self, Collo, BC, uv_layers, lb, ub, eps, c):
    ...
    def loss_func_lbfgs(self):
        ...
        loss = loss_f * loss_f.item()/self.c + loss_bc + loss_mu
...
if __name__ == "__main__":
    ...
    for c in [0.1, 0.01, 0.001, 0.0001]:
        if not os.path.exists('results_' + str(c)):
            os.mkdir('results_' + str(c))

        shutil.copy('model_based.pth', './results_' + str(c)+'/model_based.pth')
        os.chdir('results_' + str(c))

        if not os.path.exists('results'):
            os.mkdir('results')

        model = PhysicsInformedNN(x, BC, uv_layers, lb, ub, eps, c)
        model.train_lbfgs()
        ...
        df.to_csv('result_AtPINN_0.02.csv', index=False, header=False)
        os.chdir('..')
